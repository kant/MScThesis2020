...
def genomic_exon_coordinate_ranges(gene, sqlite_path_organism, supported_transcripts, filter=True):
    ...
    print gene, sqlite_path_organism, supported_transcripts, filter
    exon_info = get_exon_info(gene, sqlite_path_organism, supported_transcripts, filter=filter)
    print "genomic " + str(exon_info)
...
def genomic_orf_coordinate_ranges(gene, sqlite_path_organism, supported_transcripts, filter=True):
    genomic_coordinates = genomic_exon_coordinate_ranges(gene, sqlite_path_organism, supported_transcripts, filter=filter)
...
def genomic_junction_positions(gene, sqlite_path_organism, supported_transcripts, filter=True):

    exons = genomic_exon_coordinate_ranges(gene, sqlite_path_organism, supported_transcripts)
    genomic_junctions = {}
...
def genomic_junction_scores(gene_name, sqlite_path_organism, sqlite_path_reads, supported_transcripts, filter=True):
    genomic_junctions = genomic_junction_positions(gene_name, sqlite_path_organism, supported_transcripts, filter=filter)
...
def get_reads_per_genomic_location_asite(gene, sqlite_path_reads, sqlite_path_organism, supported_transcripts, filter=True):
...
def get_reads_per_genomic_location_fiveprime(gene, sqlite_path_reads, sqlite_path_organism, supported_transcripts,
                                             filter):
    # get the number of reads supporting each genomic position to be used in the display of support of the
    # supertranscript model. This function takes the reads mapped for each transcript of a gene and uses a combination
    # of genomic and transcriptomic ranges to translate each transcript position to a genomic one.
...
def reverse(s):
    ...
    reverse = ''.join(comp[i] for i in s)
    lst = list(reverse)
    reverse_lst = lst[::-1]
    output = ''.join(reverse_lst)
    return(output)
...
def get_scores_per_exon_for_gene(gene_name, sqlite_path_organism, sqlite_path_reads, supported_transcripts,
                                 filter=True):
    # count the reads in the reads file whos p sites lite within the exon sequence
    # returns a dictionary with all unique exons in the gene as keys and counts as values
...
def get_scores_for_gene(gene_name, sqlite_path_organism, sqlite_path_reads, supported_transcripts, filter=True):
    ...
    for transcript in transcripts:
        if transcript in exon_pileup:
            exon_pileup[transcript] += get_exon_pileup_for_transcript(transcript, sqlite_path_organism,
                                                                      sqlite_path_reads)
        else:
            exon_pileup[transcript] = get_exon_pileup_for_transcript(transcript, sqlite_path_organism,
                                                                     sqlite_path_reads)
...
def scores_per_exon_number(gene, sqlite_path_organism, sqlite_path_reads, supported_transcripts, filter=True):
    ...
    scores = get_scores_per_exon_for_gene(gene, sqlite_path_organism, sqlite_path_reads, supported_transcripts,
                                          filter=filter)
        ...
        for exon in positions_number:
            if i in range(positions_number[exon][0] - 100, positions_number[exon][1] + 100):
                ...
                if exon not in number_x_y:
                    number_x_y[exon] = [positions_number[exon][0], exons_in_range * -1]
...
def nx_plot_spring(gene, sqlite_path_organism, sqlite_path_reads, filter=True):
    graph = nx.DiGraph()
    ...
    color_lookup = scores
    sorted_color_lookup = sorted(color_lookup.values())
    low = min(sorted_color_lookup)
    high = max(sorted_color_lookup)
