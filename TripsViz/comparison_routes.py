	...
	if inputtran == False:
		...
		if result != []:
			if len(result) == 1:
				...
				rankings = ribo_seq_read_counting(tran, sqlite_path_organism, file_paths["riboseq"].values(),
												  count_type="asite", unique=True)
				...
				for transcript in result:
					...
					if cds_start == "NULL" or cds_start == None:
						...
						if transcript[0] in rankings:
							...
							coverage = "NULL"
					return_str += (":{},{},{},{},{},{}".format(transcript[0], tranlen, cds_start, cdslen, threeutrlen,coverage))
				return return_str
		else:
